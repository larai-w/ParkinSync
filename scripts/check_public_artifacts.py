...
def validate_nzbase_artifacts() -> list[str]:
    failures: list[str] = []
    source_path = WEEKLY_OUTPUT_DIR / "bundle-synthetic-weekly-transaction-bundle.json"
    try:
        source = json.loads(source_path.read_text(encoding="utf-8"))
        manifest = json.loads(NZ_BASE_MANIFEST_PATH.read_text(encoding="utf-8"))
        bundle_path = NZ_BASE_OUTPUT_DIR / manifest["bundle_file"]
        bundle = json.loads(bundle_path.read_text(encoding="utf-8"))
    except (KeyError, json.JSONDecodeError, OSError) as error:
        return [f"cannot validate NZ Base artifacts: {error}"]

    if manifest.get("schema_version") != "parkinsync-fhir-nzbase-v1":
        failures.append("NZ Base manifest schema version is not reviewed")
    if manifest.get("classification") != "synthetic":
        failures.append("NZ Base manifest must be classified as synthetic")
    if manifest.get("fhir_release") != "4.0.1":
        failures.append("NZ Base manifest must declare FHIR R4 4.0.1")
    if manifest.get("nz_base_package") != "fhir.org.nz.ig.base#3.1.0":
        failures.append("NZ Base manifest must pin the reviewed published package")
    if manifest.get("source_bundle") != source_path.as_posix():
        failures.append("NZ Base manifest source Bundle is not reviewed")
    ...
    if manifest.get("source_bundle_sha256") != hashlib.sha256(
        ...
        failures.append("NZ Base manifest source Bundle digest does not match")
    ...
    if manifest.get("sha256", {}).get(bundle_path.name) != hashlib.sha256(
        ...
        failures.append("NZ Base manifest derivative Bundle digest does not match")

    actual_names = sorted(path.name for path in NZ_BASE_OUTPUT_DIR.glob("*.json"))
    expected_names = sorted([*manifest.get("files", []), "manifest.json"])
    if actual_names != expected_names:
        failures.append("NZ Base manifest file list does not match generated artifacts")
    if bundle.get("resourceType") != "Bundle" or bundle.get("type") != "transaction":
        failures.append("NZ Base derivative must be a transaction Bundle")
    if bundle.get("id") != "synthetic-weekly-nzbase-transaction-bundle":
        failures.append("NZ Base derivative Bundle id is not reviewed")

    patient_profile = "http://hl7.org.nz/fhir/StructureDefinition/NzPatient"
    medication_profile = (
        "http://hl7.org.nz/fhir/StructureDefinition/NzMedicationStatement"
    )
    expected_profiles = {
        "Patient": [patient_profile],
        "MedicationStatement": [medication_profile],
    }
    found_counts: dict[str, int] = {}
    ...
    for entry in bundle.get("entry", []):
        ...
        if resource_type in expected_profiles:
            if profiles != expected_profiles[resource_type]:
                failures.append(f"NZ Base profile declaration is invalid: {resource_type}")
            for profile in profiles:
                profile_counts[profile] = profile_counts.get(profile, 0) + 1
        elif profiles:
            failures.append(f"unreviewed NZ Base profile declared for {resource_type}")
        if resource_type == "Patient":
            ...
            if any(
                ...
                failures.append("NZ Base synthetic Patient must not contain an NHI")
            if resource.get("extension"):
                failures.append("NZ Base synthetic Patient must not infer extensions")
        if resource_type == "MedicationStatement" and resource.get(
            "medicationCodeableConcept", {}
        ).get("coding"):
            failures.append("NZ Base synthetic medication must not infer NZMT coding")
    ...
    if found_counts != expected_counts or manifest.get("resource_type_counts") != expected_counts:
        failures.append("NZ Base derivative resource counts are not reviewed")
    if profile_counts != manifest.get("profile_counts"):
        failures.append("NZ Base profile counts do not match the manifest")

    expected = json.loads(json.dumps(source))
    expected["id"] = "synthetic-weekly-nzbase-transaction-bundle"
    ...
    if bundle != expected:
        failures.append("NZ Base derivative changes content beyond the reviewed profile overlay")

    try:
        validator = NZ_BASE_VALIDATOR_PATH.read_text(encoding="utf-8")
        workflow = CI_WORKFLOW_PATH.read_text(encoding="utf-8")
    except OSError as error:
        failures.append(f"cannot validate NZ Base CI contract: {error}")
        return failures
    required_text = (
        "fhir.org.nz.ig.base#3.1.0",
        "-tx n/a",
        "scripts/validate_fhir_nzbase.sh",
        "scripts/generate_nzbase_fhir.py --check",
    )
    combined = validator + "\n" + workflow
    if any(value not in combined for value in required_text):
        failures.append("NZ Base CI does not match the reviewed package and offline contract")
    return failures
