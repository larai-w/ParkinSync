#!/usr/bin/env python3
"""Generate or verify the deterministic synthetic NZ Base derivative Bundle."""
...
from fhir_nzbase import render_nzbase_outputs  # noqa: E402
...
def check_outputs(expected: dict[str, str]) -> bool:
    ...
    if actual_names != sorted(expected):
        print("NZ Base output file set is stale or incomplete", file=sys.stderr)
        return False
    stale = [
    ...
    if stale:
        for name in stale:
            print(f"stale NZ Base artifact: fhir/nzbase/generated/{name}", file=sys.stderr)
        return False
    print(f"Synthetic NZ Base artifacts are reproducible ({len(expected)} artifacts).")
    return True
...
def write_outputs(outputs: dict[str, str]) -> None:
    ...
    print(f"Wrote {len(outputs)} synthetic NZ Base artifacts to fhir/nzbase/generated/.")
...
def main() -> int:
    ...
    try:
        ...
        outputs = render_nzbase_outputs(source_bundle)
        if args.check:
            return 0 if check_outputs(outputs) else 1
        write_outputs(outputs)
        return 0
    except (json.JSONDecodeError, OSError, ValueError) as error:
        print(f"NZ Base generation failed: {error}", file=sys.stderr)
        return 1
